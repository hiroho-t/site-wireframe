...
| id | 背景 | 構造 |
|---|---|---|
| message-001 | 白 | 上に全幅・中央揃えの小見出し、中央にコピー画像430×209＋本文（16px lh32・幅484）＋右下にリンク |
...
| id | 背景 | 構造 |
|---|---|---|
| feature-001 | 白 | ラベル＋和文40pxの見出し。下に左：円形の画像枠φ430／右：本文＋角丸ボタン（290×62） |
...
| id | 背景 | 構造 |
|---|---|---|
| data-001 | 明グレー | 見出し＋「単位24px＋数字60px＋単位24px」のスタットブロックを左右に2つ。図版のグレーボックスを添える |
...
| id | 背景 | 構造 |
|---|---|---|
| gallery-001 | 白 | 縦長タイル197×540を2段で敷き詰める帯（高さ420）。上下は画面外にはみ出す。パララックス想定を静止表示 |
